    ...
    public class Line : LineBase, ICloneable, ISerializable {
        ...
        private void InterpolatePoint(Graphics g, GraphPane pane, CurveItem curve, PointPair lastPt,
            ...
            try {
                ...
                float minX = chartRect.Left;
                float maxX = chartRect.Right;
                // try to interpolate values
                bool lastIn = chartRect.Contains(lastX, lastY);
                bool curIn = chartRect.Contains(tmpX, tmpY);

                bool valid = !((IsOut(Edges.Left, lastX, chartRect) && IsOut(Edges.Left, tmpX, chartRect)) ||
                               (IsOut(Edges.Right, lastX, chartRect) && IsOut(Edges.Right, tmpX, chartRect)) ||
                               (IsOut(Edges.Top, lastY, chartRect) && IsOut(Edges.Top, tmpY, chartRect)) ||
                               (IsOut(Edges.Bottom, lastY, chartRect) && IsOut(Edges.Bottom, tmpY, chartRect)));
                ...
                if (valid) {
                    ...
                    if (!lastIn) {
                        ...
                        if (x < chartRect.Left) {
                            x = chartRect.Left;
                            y = tmpY + (lastY - tmpY) * (x - tmpX) / (lastX - tmpX);
                        } else if (x > chartRect.Right) {
                            x = chartRect.Right;
                            y = tmpY + (lastY - tmpY) * (x - tmpX) / (lastX - tmpX);
                        }
                        ...
                        if (y < chartRect.Top) {
                            y = chartRect.Top;
                            x = tmpX + (lastX - tmpX) * (y - tmpY) / (lastY - tmpY);
                        } else if (y > chartRect.Bottom) {
                            y = chartRect.Bottom;
                            x = tmpX + (lastX - tmpX) * (y - tmpY) / (lastY - tmpY);
                        }
        ...
        private bool IsOut(Edges edge, float value, RectangleF rect) {
            switch (edge) {
                ...
                case Edges.Right:
                    return value > rect.Right;
                case Edges.Top:
                    return value < rect.Top;
                case Edges.Bottom:
                    return value > rect.Bottom;
            }
            return false;
        }
        ...
        public bool BuildPointsArray(GraphPane pane, CurveItem curve,
            ...
            if (this.IsVisible && !this.Color.IsEmpty && points != null) {
                ...
                pointsList = new List<PointF>((_stepType == ZedGraph.StepType.NonStep ? 1 : 2) *
                                            points.Count + 1);
                ...
                for (int i = 0; i < points.Count; i++) {
                    ...
                    if (!points[i].IsInvalid) {
                        ...
                        if (_isSmooth || index == 0 || this.StepType == StepType.NonStep) {
                            pointsList.Add(new PointF(curX, curY));
                        } else if (this.StepType == StepType.ForwardStep ||
                                          this.StepType == StepType.ForwardSegment) {
                            pointsList.Add(new PointF(curX, lastY));
                            index++;
                            pointsList.Add(new PointF(curX, curY));
                        } else if (this.StepType == StepType.RearwardStep ||
                                          this.StepType == StepType.RearwardSegment) {
                            pointsList.Add(new PointF(lastX, curY));
                            index++;
                            pointsList.Add(new PointF(curX, curY));
                        }
